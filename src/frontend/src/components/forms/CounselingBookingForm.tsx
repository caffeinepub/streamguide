...
import { useSubmitCounselingSessionBooking } from '../../hooks/mutations/useSubmitCounselingSessionBooking';
...
export default function CounselingBookingForm() {
  const [formData, setFormData] = useState({
    ...
    preferredDate: '',
    preferredTimeWindow: '',
    preferredMode: 'online' as 'online' | 'in_person',
    notes: '',
  });

  const [errors, setErrors] = useState<Record<string, string>>({});
  const mutation = useSubmitCounselingSessionBooking();
  ...
  const validateForm = () => {
    ...
    if (!formData.preferredTimeWindow.trim()) newErrors.preferredTimeWindow = 'Preferred time is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const preferredModeEnum: PreferredMode =
      formData.preferredMode === 'online' ? PreferredMode.online : PreferredMode.in_person;
  ...
  if (mutation.isSuccess) {
    return (
      <SubmissionStatePanel
        ...
        message="Thank you for booking a counseling session. We'll contact you within 1-2 business days to confirm your appointment and provide further details."
    ...
    <form onSubmit={handleSubmit} className="space-y-6 bg-card border border-border rounded-xl p-6 md:p-8">
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-4 py-2 border border-input rounded-lg bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-amber-600"
        />
        {errors.preferredDate && <p className="mt-1 text-sm text-destructive">{errors.preferredDate}</p>}
      </div>

      {/* Preferred Time Window */}
      <div>
        <label htmlFor="preferredTimeWindow" className="block text-sm font-medium text-foreground mb-2">
        ...
        {errors.preferredTimeWindow && <p className="mt-1 text-sm text-destructive">{errors.preferredTimeWindow}</p>}
      </div>

      {/* Preferred Mode */}
      <div>
        <label htmlFor="preferredMode" className="block text-sm font-medium text-foreground mb-2">
      ...
      >
        {mutation.isPending ? 'Submitting...' : 'Book Counseling Session'}
      </button>
    </form>
  );
}
